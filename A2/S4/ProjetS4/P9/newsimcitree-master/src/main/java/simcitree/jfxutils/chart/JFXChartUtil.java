...
import simcitree.jfxutils.JFXUtil;
...
public class JFXChartUtil {
	/**
	 * Sets up zooming via the {@link #setupZooming(ScatterChart, EventHandler)} method by using the
	 * {@link ChartZoomManager}'s {@link ChartZoomManager#DEFAULT_FILTER default filter}.
	 */
	public static Region setupZooming( ScatterChart<?, ?> chart ) {
	...
	/**
	 * Calls {@link #addDoublePrimaryClickAutoRangeHandler(ScatterChart, Node)} with the chart as the target.
	 */
	public static EventHandler<MouseEvent> addDoublePrimaryClickAutoRangeHandler( final ScatterChart<?, ?> chart ) {
